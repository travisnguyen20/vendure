...
import { ADMIN_API_PATH, API_PORT, SHOP_API_PATH, VENDOR_API_PATH } from '../../packages/common/src/shared-constants';
...
const CLIENT_VENDOR_QUERY_FILES = path.join(__dirname, '../../packages/admin-ui/src/lib/core/src/data/vendor-definitions/**/*.ts');
...
const VENDOR_SCHEMA_OUTPUT_FILE = path.join(__dirname, '../../schema-vendor.json');
...
Promise.all([
    ...
    downloadIntrospectionSchema(VENDOR_API_PATH, VENDOR_SCHEMA_OUTPUT_FILE),
])
    .then(([adminSchemaSuccess, shopSchemaSuccess, vendorSchemaSuccess]) => {
        if (!adminSchemaSuccess || !shopSchemaSuccess || !vendorSchemaSuccess ) {
        ...
        const vendorSchemaJson = JSON.parse(fs.readFileSync(VENDOR_SCHEMA_OUTPUT_FILE, 'utf-8'));
        const adminSchema = buildClientSchema(adminSchemaJson.data);
        const shopSchema = buildClientSchema(shopSchemaJson.data);
        const vendorSchema = buildClientSchema(vendorSchemaJson.data);
        ...
        return generate({
            ...
            generates: {
                ...
                [path.join(__dirname, '../../packages/admin-ui/src/lib/core/src/common/generated-types.ts')]: {
                    schema: [ADMIN_SCHEMA_OUTPUT_FILE, VENDOR_SCHEMA_OUTPUT_FILE, path.join(__dirname, 'client-schema.ts')],
                    documents: CLIENT_QUERY_FILES,
                    plugins: clientPlugins,
                    config,
                },
                [path.join(__dirname, '../../packages/admin-ui/src/lib/core/src/common/generated-vendor-types.ts')]: {
                    schema: [VENDOR_SCHEMA_OUTPUT_FILE],
                    documents: CLIENT_VENDOR_QUERY_FILES,
                    plugins: clientPlugins,
                    config,
                },
                ...
                [path.join(__dirname, '../../packages/admin-ui/src/lib/core/src/common/introspection-vendor-result.ts')]: {
                    schema: [VENDOR_SCHEMA_OUTPUT_FILE],
                    documents: CLIENT_VENDOR_QUERY_FILES,
                    plugins: [{ add: '// tslint:disable' }, 'fragment-matcher'],
                    config,
                ...
                [path.join(__dirname, '../../packages/common/src/generated-vendor-types.ts')]: {
                    schema: [VENDOR_SCHEMA_OUTPUT_FILE],
                    plugins: commonPlugins,
                    config,
                },
